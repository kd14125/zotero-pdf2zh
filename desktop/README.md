...
MinerU 的 `layout.json` 同时用于两类增强：行间公式坐标继续补充 PDF2ZH 漏检区域；包含分式、
积分、求和等结构的复杂行内公式会合并为一个 BabelDOC 排版单元。公式以真实矢量宽度、高度和
基线参与排版，生成 PDF 后再清理落点中的旧公式残片并写回矢量对象。

默认渲染器为随应用打包的 MathJax 4，不需要系统安装 Node.js 或 LaTeX。设置页也提供“本机
LaTeX”模式、重新检测和安装入口；Windows 自动安装通过 `winget` 获取 MiKTeX。LaTeX 使用
`pdflatex -no-shell-escape` 在任务临时目录编译，并拒绝 `input`、`write18`、`openout` 等危险
命令。MathJax 单个公式失败时，若检测到 `pdflatex`，运行器可逐公式回退。
