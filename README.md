...
[![Desktop release](https://img.shields.io/badge/Desktop-v0.3.2-2E7D32?style=flat-square)](https://github.com/kd14125/pdf2zh-desktop/releases/tag/desktop-v0.3.2)
...
[![直接下载 Windows 安装包](https://img.shields.io/badge/直接下载-Windows%20安装包-2E7D32?style=for-the-badge&logo=windows&logoColor=white)](https://github.com/kd14125/pdf2zh-desktop/releases/download/desktop-v0.3.2/PDF2ZH-Desktop-Setup-0.3.2.exe)

**普通用户只需下载并运行 `.exe`，不需要下载 `.sha256` 文件。**

[查看版本说明](https://github.com/kd14125/pdf2zh-desktop/releases/tag/desktop-v0.3.2)
· [SHA-256 校验文件（高级用户可选）](https://github.com/kd14125/pdf2zh-desktop/releases/download/desktop-v0.3.2/PDF2ZH-Desktop-Setup-0.3.2.exe.sha256)

</div>

> [!TIP]
> **v0.3.2 新增 MinerU 公式漏检增强。** MinerU 只提供公式坐标提示，PDF2ZH 会与自身识别结果
> 比较并补充漏检的行间公式，最终仍由 BabelDOC 保留原 PDF 公式对象和版式，不使用 MinerU
> LaTeX 重新绘制公式。既可在翻译前启用，也可从历史记录找到原始 PDF 后重新生成增强版。
...
- 可选 MinerU 公式漏检增强，只补充 PDF2ZH 漏检的行间公式坐标，不接管正文翻译和 PDF 排版
...
**[直接下载 PDF2ZH Desktop 安装包（`.exe`）](https://github.com/kd14125/pdf2zh-desktop/releases/download/desktop-v0.3.2/PDF2ZH-Desktop-Setup-0.3.2.exe)**
...
需要优化公式漏检时，在“设置”页面的“MinerU 公式漏检增强”区域填写 API 地址和 Token，选择
`vlm` 或 `pipeline` 模型后测试连接并保存。Token 可在
[MinerU Token 管理页](https://mineru.net/apiManage/token)登录后创建或复制，保存后使用 Windows
DPAPI 加密存储在当前电脑。

回到“翻译”页面，勾选“MinerU 公式漏检增强”即可在翻译前进行公式坐标分析。已有翻译任务可在
